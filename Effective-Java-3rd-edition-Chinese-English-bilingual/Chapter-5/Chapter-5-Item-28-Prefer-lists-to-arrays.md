...
编译器告诉你，它不能保证在运行时转换的安全性，因为程序不知道类型 T 代表什么。记住，**元素类型信息在运行时从泛型中删除。** *是不是因为这样强转才成功*这个计划会奏效吗？是的，但是编译器不能证明它。你可以向自己证明这一点，但是你最好将证据放在注释中，指出消除警告的原因（[Item-27](/Chapter-5/Chapter-5-Item-27-Eliminate-unchecked-warnings.md)），并使用注解隐藏警告。
